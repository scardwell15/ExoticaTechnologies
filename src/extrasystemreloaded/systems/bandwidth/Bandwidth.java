...
public enum Bandwidth {
    ...
    ULTIMATE(200f, "pristine", new Color(150,100,200), 8),
    PERFECT(250f, "ultimate", new Color(255,100,255), 4),
    ...
    public static Bandwidth generate(long seed) {
        ...
        log.info(String.format("Bandwidth weight generated: [%s]", chosen));
        for(Bandwidth b : values()) {
            chosen -= b.getWeight();

            if(chosen <= 0) {
                log.info(String.format("Bandwidth generated: [%s]", b.getKey()));
                return b;
            }
        }
        return DOMAIN;
