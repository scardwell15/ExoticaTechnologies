package exoticatechnologies.combat.particles
...
class ParticleController : BaseCombatLayeredRenderingPlugin() {
    val PARTICLE_RENDER_LAYER = EnumSet.of(CombatEngineLayers.CONTRAILS_LAYER)
    var PARTICLES: MutableList<ParticleData> = mutableListOf()
    ...
    companion object {
        val INSTANCE = ParticleController()
    }
}
...
open class ParticleData(
    ...
    fun getColor(): Color {
        return RenderUtils.mergeColors(startingColor, endColor, ((elapsedRatio() - 0.66f) * 3f).coerceIn(0f..1f))
    }
}