...
abstract class DefaultUpgradeMethod : UpgradeMethod {
     ...
     */
    override fun canUse(member: FleetMemberAPI, variant: ShipVariantAPI, mods: ShipModifications, upgrade: Upgrade, market: MarketAPI?): Boolean {
        return upgrade.canApplyImpl(member, variant, mods) && usesLevel() && mods.getUpgrade(upgrade) + 1 <= upgrade.maxLevel
                && usesBandwidth()
