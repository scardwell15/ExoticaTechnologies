...
abstract class FilteredTabListPanel<I, T : PanelContext>(context: TabListContext<I, T>) : TabListPanel<I, T>(context) {
    val selectedFilters: MutableList<String>
        get() = (currContext as FilteredTabListContext<I, T>).selectedFilters
    private val filterCheckboxes: MutableList<ButtonAPI> = mutableListOf()

    override fun createListContainer(menuPanel: CustomPanelAPI, context: TabListContext<I, T>): CustomPanelAPI {
        val extraPadding = getListPanelPositionPadding()
        val scrollPanelWidth = itemWidth + 4f

        val listContainer =
            menuPanel.createCustomPanel(scrollPanelWidth, innerHeight - extraPadding.y, context.tabsContainerPlugin)
        ...
        val tabsToShow = context.tabs.filter { shouldAllowItem(it.item) }

        val innerTooltip = listContainer.createUIElement(scrollPanelWidth, innerHeight - extraPadding.y - 22f, true)
        ...
        menuPanel.addComponent(listContainer).inTL(extraPadding.x + innerPadding, extraPadding.y + innerPadding)
    ...
    override fun finishedRefresh(
        ...
        context: TabListContext<I, T>,
        listItems: List<TabListItem<I, T>>
    ) {
    ...
    open fun createFilterPanel(menuPanel: CustomPanelAPI) {
        ...
        filterPanelLocal.addUIElement(filterTooltip).inBL(innerPadding + 2f, 0f)
        menuPanel.addComponent(filterPanelLocal).inBL(innerPadding + 2f, 22f + innerPadding)
    }
...
open class FilteredTabListContext<I, T : PanelContext> : TabListContext<I, T>() {
